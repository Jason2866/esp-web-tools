# LittleFS Filesystem Manager for ESP Web Tools

## Overview

ESP Web Tools now supports detection and management of LittleFS partitions on ESP32/ESP8266 devices.

## Features

### Automatic Filesystem Detection
- Automatic detection of LittleFS partitions in the partition table
- Distinction between LittleFS and SPIFFS through signature recognition
- Support for various block sizes (512, 1024, 2048, 4096 bytes)

### File Management
- **Display Files**: Hierarchical view of all files and folders
...
- Display of LittleFS disk version (v2.0, v2.1)
- Partition size and name
- Free/used storage

### Backup & Restore
- **Backup**: Save complete filesystem image as .bin file
- Image can be restored later with "Read Flash"
...
3. This can be restored later with "Read Flash"
...
Automatic detection uses multiple methods:

#### Method 1: String Signature (Primary)
- LittleFS stores the string "littlefs" in superblock metadata
...
- SPIFFS is older and more common, hence the safe default
- Error messages are logged for debugging

### Integration with littlefs-wasm
The implementation uses the `littlefs-wasm` package, which compiles LittleFS to WebAssembly and runs it in the browser. All operations occur locally in browser memory.

## Limitations

- **Read Only**: Changes are only made locally in the browser
- **No Direct Writing**: To transfer changes to the device, the complete image must be reflashed
...
7. Flash the saved image back to the device with "Program"

## Troubleshooting

### "Failed to mount LittleFS with any block size"
- The partition may not be formatted
- The partition may be using SPIFFS instead of LittleFS
...
The LittleFS functionality is integrated in ESP Web Tools as an optional feature. It is automatically activated when a partition with a filesystem is detected.
...
The LittleFS functionality is automatically available when the device is connected and a partition table is read.
